...
router.get('/', async (req, res) => {
  const staff = await prisma.staff.findMany({ include: { site: true, supplier: true }, orderBy: { createdAt: 'desc' } })
  res.json(staff)
})

router.get('/:id', async (req, res) => {
  const member = await prisma.staff.findUnique({ where: { id: req.params.id }, include: { site: true, supplier: true } })
...
router.post('/', async (req, res) => {
  ...
  const member = await prisma.staff.create({ data: { name, hireType, siteId: siteId || null, position, mobile, email, startDate, supplierId: supplierId || null, role, checklist }, include: { site: true, supplier: true } })
...
router.patch('/:id', async (req, res) => {
  ...
  if (siteId !== undefined && checklist === undefined) {
    ...
    if (siteId) {
      const site = await prisma.site.findUnique({ where: { id: siteId } })
      data.checklist = site ? applySiteInductions(newChecklist, site) : newChecklist
    } else {
      data.checklist = newChecklist
    }
  }
  const member = await prisma.staff.update({ where: { id: req.params.id }, data, include: { site: true, supplier: true } })
  res.json(member)
})
